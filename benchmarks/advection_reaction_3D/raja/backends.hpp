...
using XYZ_KERNEL_SERIAL_POLICY = RAJA::KernelPolicy<RAJA::statement::For<
  2, RAJA::seq_exec,
  RAJA::statement::For<
    1, RAJA::seq_exec,
...
using EXEC_POLICY    = RAJA::cuda_exec<256, false>;
using XYZ_KERNEL_POL = RAJA::KernelPolicy<RAJA::CudaKernel<RAJA::statement::For<
...
static int EnableFusedVectorOps(N_Vector y)
{
  ...
  if (check_retval(&retval, "N_VEnableFusedOps_MPIPlusX", 1, myid))
  {
    return (-1);
  }
  ...
  if (check_retval(&retval, "N_VEnableFusedOps_OpenMPDEV", 1, myid))
    return (-1);
#else
  retval = N_VEnableFusedOps_Serial(N_VGetLocalVector_MPIPlusX(y), 1);
  if (check_retval(&retval, "N_VEnableFusedOps_Serial", 1, myid))
  {
    return (-1);
  }
#endif
