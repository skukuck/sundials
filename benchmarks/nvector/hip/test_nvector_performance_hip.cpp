 ...
 * NVECTOR HIP module implementation.
 * -----------------------------------------------------------------*/
                                 ...
                                 sunrealtype* out)
{
  ...
  while (i != 0)
  {
    if (sharedidx < i) shared[sharedidx] += shared[sharedidx + i];
    __syncthreads();
    i /= 2;
  }

  if (sharedidx == 0) out[sharedidx] = shared[0];
}
...
{
  rand_realtype_zero_one(N_VGetHostArrayPointer_Hip(Xvec), Xlen);
  N_VCopyToDevice_Hip(Xvec);
}
...
{
  rand_realtype_constraints(N_VGetHostArrayPointer_Hip(Xvec), Xlen);
  N_VCopyToDevice_Hip(Xvec);
}
...
static int InitializeClearCache(int cachesize)
{
  ...
  err = hipMalloc((void**)&d_data, N * sizeof(sunrealtype));
  if (err != hipSuccess)
  {
  ...
  err = hipMalloc((void**)&d_sum, blocksPerGrid * sizeof(sunrealtype));
  if (err != hipSuccess)
  {
    fprintf(stderr, "Failed to allocate device vector (error code %d )!\n", err);
    return (-1);
  ...
  err = hipMemcpy(d_data, h_data, N * sizeof(sunrealtype), hipMemcpyHostToDevice);
  if (err != hipSuccess)
  {
    fprintf(stderr,
            "Failed to copy data from host to device (error code %d )!\n", err);
    return (-1);
  }
...
void ClearCache()
{
  /* call hip kernel to clear the cache */
  ClearCacheKernel<<<SUNMIN(32, (N + 255) / 256), 256>>>(N, d_data, d_sum);
  hipMemcpy(h_sum, d_sum, blocksPerGrid * sizeof(sunrealtype),
