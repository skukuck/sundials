...
def adapt_sundials_types_returns_to_shared_ptr(
    ...
    if str(adapted_function.cpp_adapted_function.return_type) not in sundials_pointer_types:
        return None
    ...
    for param in new_function_params:
        lambda_adapter.adapted_cpp_parameter_list.append(f"{param.cpp_element().decl.decl_name}")

    lambda_adapter.lambda_name = (
        adapted_function.cpp_adapted_function.function_name + f"_adapt_return_type_to_shared_ptr"
    )
