...
application of the BDF :eq:`IDA_BDF` to the DAE system :eq:`IDA_DAE` results in
...
By default IDA solves :eq:`IDA_DAE_nls` with a Newton iteration but IDA also
allows for user-defined nonlinear solvers (see Chapter :numref:`SUNNonlinSol`).
Each Newton iteration requires the solution of a linear system of the form
...
.. _IDA.Mathematics.step_order_select:
