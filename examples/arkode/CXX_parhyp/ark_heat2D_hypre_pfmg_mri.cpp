/* -----------------------------------------------------------------------------
 * Programmer(s): Rujeko Chinomona @ UMBC
...
static int ff(sunrealtype t, N_Vector u, N_Vector f, void* user_data);
...
int main(int argc, char* argv[])
{
  ...
  (SUN_HYPRE_VERSION_MAJOR == 2 && SUN_HYPRE_VERSION_MINOR >= 20)
    ...
    // Specify tolerances
    flag = ARKodeSStolerances(inner_arkode_mem, udata.rtol, udata.atol);
    if (check_flag(&flag, "ARKodeSStolerances", 1)) { return 1; }

    // Attach linear solver
    flag = ARKodeSetLinearSolver(inner_arkode_mem, LSf, NULL);
    if (check_flag(&flag, "ARKodeSetLinearSolver", 1)) { return 1; }

    // Set linear solver tolerance factor
    flag = ARKodeSetEpsLin(inner_arkode_mem, udata.epslin);
    if (check_flag(&flag, "ARKodeSetEpsLin", 1)) { return 1; }

    // Use an ARKode provided table
    flag = ARKodeSetOrder(inner_arkode_mem, udata.forder);
    if (check_flag(&flag, "ARKodeSetOrder", 1)) { return 1; }

    // Set fixed step size or adaptivity method
    if (udata.hf > ZERO)
    {
      flag = ARKodeSetFixedStep(inner_arkode_mem, udata.hf);
    ...
    // Attach user data
    flag = ARKodeSetUserData(inner_arkode_mem, &udata);
    ...
    // Set fixed step size
    flag = ARKodeSetFixedStep(arkode_mem, udata.hs);
    if (check_flag(&flag, "ARKodeSetFixedStep", 1)) { return 1; }
...
static int fe(sunrealtype t, N_Vector u, N_Vector f, void* user_data)
{
  ...
  sunrealtype cxm = -SUNMIN(udata->ax, ZERO) / (udata->dx);
  sunrealtype cyp = -SUNMAX(udata->ay, ZERO) / (udata->dy);
  sunrealtype cym = -SUNMIN(udata->ay, ZERO) / (udata->dy);
...
static int FillMatrix(UserData* udata, sunrealtype gamma)
{
  ...
  if ((ilower[0] <= iupper[0]) && (ilower[1] <= iupper[1]))
  {
    ...
    sunrealtype c2 = udata->ky / (udata->dy * udata->dy);
    sunrealtype c3 = -TWO * (c1 + c2);

    // Linear system values
    sunrealtype cx = -gamma * c1;
    sunrealtype cy = -gamma * c2;
    sunrealtype cc = ONE - gamma * c3;
...
static int ReadInputs(int* argc, char*** argv, UserData* udata, bool outproc)
{
  ...
  while (arg_idx < (*argc))
  {
    ...
    else if (arg == "--sorder") { udata->sorder = stoi((*argv)[arg_idx++]); }
