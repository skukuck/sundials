...
static int BSend(MPI_Comm comm, int thispe, int ixsub, int jysub, int npex,
                 ...
                 const sunrealtype uarray[], sunrealtype* send_buffer);
...
static int BRecvPost(MPI_Comm comm, MPI_Request request[], int thispe, int ixsub,
                     ...
                     sunindextype mysub, sunrealtype recv_buff[]);

static int BRecvWait(MPI_Request request[], int ixsub, int jysub, int npex,
                     int npey, sunindextype mxsub, sunindextype mysub,
                     sunrealtype uext[], const sunrealtype recv_buff[]);
                 ...
                 const sunrealtype uarray[], sunrealtype* send_buffer)
{
  ...
  sunrealtype* bufleft   = send_buffer;
  sunrealtype* bufright  = send_buffer + mysub;
  sunrealtype* buftop    = send_buffer + 2 * mysub;
  sunrealtype* bufbottom = send_buffer + 2 * mysub + mxsub;

  /* If jysub > 0, send data from bottom x-line of u.  (via bufbottom) */

  if (jysub != 0)
  {
    for (lx = 0; lx < mxsub; ++lx) { bufbottom[lx] = uarray[lx]; }
  ...
  if (ixsub != 0)
  {
    for (ly = 0; ly < mysub; ly++) { bufleft[ly] = uarray[ly * mxsub]; }
                     ...
                     sunindextype mysub, sunrealtype recv_buff[])
{
  ...
  sunrealtype* bufleft   = recv_buff;
  sunrealtype* bufright  = recv_buff + mysub;
  sunrealtype* buftop    = recv_buff + 2 * mysub;
  sunrealtype* bufbottom = recv_buff + 2 * mysub + mxsub;

  /* If jysub > 0, receive data for bottom x-line of uext. */
  if (jysub != 0)
  {
    MPI_Irecv(bufbottom, (int)mxsub, MPI_SUNREALTYPE, thispe - npex, 0, comm,
              &request[0]);
  }
  ...
  if (jysub != npey - 1)
  {
    MPI_Irecv(buftop, (int)mxsub, MPI_SUNREALTYPE, thispe + npex, 0, comm,
              &request[1]);
  }
  ...
  if (ixsub != 0)
  {
    MPI_Irecv(&bufleft[0], (int)mysub, MPI_SUNREALTYPE, thispe - 1, 0, comm,
              &request[2]);
  }
  ...
  if (ixsub != npex - 1)
  {
    MPI_Irecv(&bufright[0], (int)mysub, MPI_SUNREALTYPE, thispe + 1, 0, comm,
              &request[3]);
  }
...
{
  MPI_Status status;
  const sunrealtype* bufleft   = recv_buff;
  const sunrealtype* bufright  = recv_buff + mysub;
  const sunrealtype* buftop    = recv_buff + 2 * mysub;
  const sunrealtype* bufbottom = recv_buff + 2 * mysub + mxsub;
...
static int DeleteUserData(UserData data)
{
  ...
  if (data->uext != NULL) { free(data->uext); }
  if (data->send_buff != NULL) { free(data->send_buff); }
  if (data->recv_buff != NULL) { free(data->recv_buff); }
  return 0;
}
