...
int main(int argc, char* argv[])
{
  ...
  if (sensi)
  {
    ...
    if (check_retval((void*)uuS, "N_VCloneVectorArray", 0, thispe))
    {
      MPI_Abort(comm, 1);
    }
    ...
    if (check_retval((void*)upS, "N_VCloneVectorArray", 0, thispe))
    {
      MPI_Abort(comm, 1);
    }
  ...
  if (thispe == 0)
  {
    PrintHeader(Neq, rtol, atol, mudq, mukeep, sensi, sensi_meth, err_con);
  }
  ...
  for (tout = t1, iout = 1; iout <= NOUT; iout++, tout *= TWO)
  {
    ...
    if (sensi)
    {
      ...
      if (check_retval(&retval, "IDAGetSens", 1, thispe))
      {
        MPI_Abort(comm, 1);
      }
    }
                    ...
                    N_Vector up, N_Vector res, void* user_data)
{
  ...
  for (ly = jybegin; ly <= jyend; ly++)
  {
    for (lx = ixbegin; lx <= ixend; lx++)
    {
      ...
      termx = p1 * data->coeffx *
              (uext[locue - 1] - TWO * uext[locue] + uext[locue + 1]);
      termy = p2 * data->coeffy *
              (uext[locue - mxsub2] - TWO * uext[locue] + uext[locue + mxsub2]);
...
static void PrintHeader(sunindextype Neq, sunrealtype rtol, sunrealtype atol,
                        ...
                        sunbooleantype sensi, int sensi_meth, int err_con)
{
  printf("\nidasHeat2D_FSA_kry_bbd_p: Heat equation, parallel example problem "
         "for IDA\n");
