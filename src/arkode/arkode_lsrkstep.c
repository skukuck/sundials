...
int lsrkStep_TakeStepSSPs2(ARKodeMem ark_mem, sunrealtype* dsmPtr, int* nflagPtr)
{
  ...
  *dsmPtr   = ZERO;
  ...
  if (!ark_mem->fn_is_current)
  {
    retval = step_mem->fe(ark_mem->tn, ark_mem->ycur, ark_mem->fn,
                          ark_mem->user_data);
    step_mem->nfe++;
    if (retval != ARK_SUCCESS)
    {
  ...
  for (int j = 2; j < step_mem->req_stages; j++)
  {
    ...
    SUNLogInfo(ARK_LOGGER, "begin-stages-list",
               "stage = %i, tcur = " SUN_FORMAT_G, j,
               ark_mem->tcur);
    N_VLinearSum(ONE, ark_mem->ycur, hsm1inv, ark_mem->tempv2,
                 ark_mem->ycur);
    if (!ark_mem->fixedstep)
    {
      N_VLinearSum(ONE, ark_mem->tempv1, bt2, ark_mem->tempv2,
                   ark_mem->tempv1);
    }
  ...
  if (!ark_mem->fixedstep)
  {
    N_VLinearSum(ONE, ark_mem->tempv1, bt3, ark_mem->tempv2,
                 ark_mem->tempv1);
...
int lsrkStep_TakeStepSSPs3(ARKodeMem ark_mem, sunrealtype* dsmPtr, int* nflagPtr)
{
  ...
  *dsmPtr   = ZERO;
  ...
  for (int j = 2; j <= ((in - 1) * (in - 2) / 2); j++)
  {
    ...
    ark_mem->tcur = ark_mem->tn + ((sunrealtype)j - ONE) * hrat;
...
int lsrkStep_TakeStepSSP104(ARKodeMem ark_mem, sunrealtype* dsmPtr, int* nflagPtr)
{
  ...
  *dsmPtr   = ZERO;
  ...
  for (int j = 2; j <= 5; j++)
  {
    ...
    N_VLinearSum(ONE, ark_mem->ycur, hsixth, ark_mem->tempv3,
                 ark_mem->ycur);
  ...
  for (int j = 6; j <= 9; j++)
  {
    ...
    {
      N_VLinearSum(ONE, ark_mem->tempv1, hfifth, ark_mem->tempv3, ark_mem->tempv1);
    }
