                       ...
                       SUNDIALS_MAYBE_UNUSED N_Vector vtemp3)
{
  ...
  {
    cvdiag_mem->formy_fused(cv_mem->h, r, fpred, cv_mem->zn[1], ypred, ftemp, y);
  }
  else
#endif
  {
  ...
  if (cv_mem->cv_usefused)
  {
    cvdiag_mem->buildM_fused(FRACT, cv_mem->uround, cv_mem->h, ftemp, fpred, cv_mem->ewt, cv_mem->di_bit, cv_mem->di_bitcomp, y, cv_mem->di_M);
  }
  else
#endif
  {
...
#ifdef SUNDIALS_BUILD_PACKAGE_FUSED_KERNELS
    if (cv_mem->cv_usefused) { cvdiag_mem->updateM_fused(r, cvdiag_mem->di_M); }
    else
#endif
    {
