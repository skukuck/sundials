...
int IDAAdjReInit(void* ida_mem)
{
  ...
  while (IDAADJ_mem->ck_mem != NULL) { IDAAckpntDelete(&(IDAADJ_mem->ck_mem)); }
...
int IDAQuadInitBS(void* ida_mem, int which, IDAQuadRhsFnBS rhsQS, N_Vector yQB0)
{
  IDAadjMem IDAADJ_mem;
  IDAMem IDA_mem;
  IDABMem IDAB_mem;
                ...
                N_Vector yp0, N_Vector* yyS0, N_Vector* ypS0)
{
  ...
  retval = N_VScaleVectorArray(IDA_mem->ida_Ns, IDA_mem->ida_cvals, yyS0,
...
int IDASolveB(void* ida_mem, sunrealtype tBout, int itaskB)
{
  ...
  {
    IDAProcessError(IDA_mem, IDA_NO_BCK, __LINE__, __func__, __FILE__,
                    MSGAM_NO_BCK);
    SUNDIALS_MARK_FUNCTION_END(IDA_PROFILER);
    return (IDA_NO_BCK);
  }
  ...
  if (IDAADJ_mem->ia_firstIDAFcall)
  {
    IDAProcessError(IDA_mem, IDA_NO_FWD, __LINE__, __func__, __FILE__,
                    MSGAM_NO_FWD);
    SUNDIALS_MARK_FUNCTION_END(IDA_PROFILER);
    return (IDA_NO_FWD);
  }
  ...
  if (IDAADJ_mem->ia_firstIDABcall)
  {
    ...
    while (tmp_IDAB_mem != NULL)
    {
      ...
      if ((sign * (tBn - IDAADJ_mem->ia_tinitial) < ZERO) ||
          (sign * (IDAADJ_mem->ia_tfinal - tBn) < ZERO))
      {
        IDAProcessError(IDA_mem, IDA_BAD_TB0, __LINE__, __func__, __FILE__,
      ...
      {
        IDAProcessError(IDA_mem, IDA_ILL_INPUT, __LINE__, __func__, __FILE__,
                        MSGAM_BAD_TBOUT, tmp_IDAB_mem->ida_index);
        SUNDIALS_MARK_FUNCTION_END(IDA_PROFILER);
        return (IDA_ILL_INPUT);
      }

      if (tmp_IDAB_mem->ida_res_withSensi || tmp_IDAB_mem->ida_rhsQ_withSensi)
      {
  ...
  if ((sign * (tBout - IDAADJ_mem->ia_tinitial) < ZERO) ||
      (sign * (IDAADJ_mem->ia_tfinal - tBout) < ZERO))
  {
    tfuzz = HUNDRED * IDA_mem->ida_uround *
            (SUNRabs(IDAADJ_mem->ia_tinitial) + SUNRabs(IDAADJ_mem->ia_tfinal));
    if ((sign * (tBout - IDAADJ_mem->ia_tinitial) < ZERO) &&
        (SUNRabs(tBout - IDAADJ_mem->ia_tinitial) < tfuzz))
    {
  ...
  for (;;)
  {
    ...
    while (tmp_IDAB_mem != NULL)
    {
      ...
      if ((tBn == ck_mem->ck_t0) && (itaskB == IDA_ONE_STEP))
      {
        isActive = SUNFALSE;
      }
      ...
      if (isActive)
      {
        ...
        flag = IDASolve(tmp_IDAB_mem->IDA_mem, tBout, &tBret,
                        tmp_IDAB_mem->ida_yy, tmp_IDAB_mem->ida_yp, itaskB);
    ...
    if (flag < 0)
    {
      IDAProcessError(IDA_mem, flag, __LINE__, __func__, __FILE__,
...
static void IDAAckpntDelete(IDAckpntMem* ck_memPtr)
{
  ...
  if (*ck_memPtr != NULL)
  {
    ...
    *ck_memPtr = (*ck_memPtr)->ck_next;
    ...
    for (j = 0; j < tmp->ck_phi_alloc; j++) { N_VDestroy(tmp->ck_phi[j]); }

    /* free N_Vectors for quadratures in tmp */
    if (tmp->ck_quadr)
    {
      for (j = 0; j < tmp->ck_phi_alloc; j++) { N_VDestroy(tmp->ck_phiQ[j]); }
    }
...
static void IDAAckpntCopyVectors(IDAMem IDA_mem, IDAckpntMem ck_mem)
{
  ...
  if (ck_mem->ck_sensi)
  {
    ...
    (void)N_VScaleVectorArray(ck_mem->ck_phi_alloc * IDA_mem->ida_Ns,
                              IDA_mem->ida_cvals, IDA_mem->ida_Xvecs,
                              IDA_mem->ida_Zvecs);
  }
                   ...
                   void* ida_mem)
{
  ...
  {
    retval = IDAB_mem->ida_res(tt, IDAADJ_mem->ia_yyTmp, IDAADJ_mem->ia_ypTmp,
                               yyB, ypB, rrB, IDAB_mem->ida_user_data);
  }
  return (retval);
}
