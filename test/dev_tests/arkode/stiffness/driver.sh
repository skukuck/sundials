...
for beta in $(seq $BETA_START $BETA_STEP $BETA_END); do
    ...
    output_file="${OUTPUT_DIR}/${test_case}.dat"
    echo -e "  Running: beta=${beta}, gamma=${FIXED_GAMMA} -> ${output_file}"
    export SUNLOGGER_INFO_FILENAME="${OUTPUT_DIR}/${test_case}.log"

    $EXECUTABLE -b $beta -g $FIXED_GAMMA -o "${output_file}"

    if [ $? -eq 0 ]; then
        echo -e "  ${GREEN}Success${NC}"
        ./plot_shampine.py "${output_file}" --save "${OUTPUT_DIR}/${test_case}.pdf"
    else
...
echo ""
echo "File summary:"
ls -lh "$OUTPUT_DIR"/*.dat 2>/dev/null | awk '{print "  " $9 " (" $5 ")"}'
