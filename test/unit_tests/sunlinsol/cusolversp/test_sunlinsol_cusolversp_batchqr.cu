...
int main(int argc, char* argv[])
{
  ...
  if (argc < 4)
  {
    printf("ERROR: THREE (3) Inputs required: matrix block size, number of "
           "blocks, print timing \n");
    return (-1);
  }
  ...
  printf("\ncuSolverSp linear solver test: size %ld, block size %ld, number of "
         "blocks %ld\n\n",
  ...
  dA = SUNMatrix_cuSparse_NewBlockCSR(nblocks, block_size, block_size,
                                      block_nnz, cusp_handle, sunctx);
  if (dA == NULL) { printf("ERROR: could not create dA\n"); }
  ...
  fails += Test_SUNLinSolSolve(LS, dA, d_x, d_b, 1000 * SUN_UNIT_ROUNDOFF,
                               SUNTRUE, 0);
